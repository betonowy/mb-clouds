...
float SecondaryRay(in vec3 ro, in float localDensity, in float phaseFunc, in float dotVal, float cloudHeight) {
    ...
    { // random length integral
        const float rayLength = SampleBlueNoise(ivec2(gl_FragCoord + 3)).x * u_SceneData.secondaryRayLength;
        RayAdvance(ro, rd, rayLength);
        float localDensity = VDB_GetVoxelAccNoisy(ro).density;

        firstIntegral = localDensity * rayLength;
    }

    while(InsideAABB(ro)) {
        RayAdvance(ro, rd, u_SceneData.secondaryRayLength);
        integral += VDB_GetVoxelAccNoisy(ro).density * u_SceneData.secondaryRayLength;
    }

    if (firstIntegral <= 0) firstIntegral = 0.001;

    integral += firstIntegral;
    integral *= u_SceneData.densityMultiplier * u_SceneData.ss_integral_mult / 2.0;
...
float MultiscatterRay(in vec3 ro, in float localDensity, in float distance, in float phaseFunc, in float dotVal, float cloudHeight) {
    ...
    integral += firstIntegral;
    integral += distance * (localDensity + value.density) * 0.5;
...
float MultiScattering(in vec3 ro, in float localDensity, in float phaseFunc, in float dotVal, float cloudHeight) {
    ...
    // main single scattering influence
    ss_Sum += SecondaryRay(ro, localDensity, phaseFunc, dotVal, cloudHeight);
