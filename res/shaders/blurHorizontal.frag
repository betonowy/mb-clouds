...
layout(location = 0) out vec4 fbCloudBlurX;

in vec2 ScreenCoord;

uniform sampler2D blueNoiseSampler;

uniform sampler2D fbCloudColor;
...
void main() {
    ...
    float dotsize = 1.0f / float(u_SceneData.windowRes.x);

    for (int i = -radius; i <= radius; i++) {
        fbCloudBlurX += getGaussian(i) * texture(fbCloudColor, coord - vec2(i * dotsize, 0));
    }
